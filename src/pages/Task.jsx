...
const Task = () => {
  ...
  useEffect(() => {
    if (mode === "update") {
      const config = { url: `/api/task/${taskId}`, method: "get", headers: { Authorization: `Bearer ${authState.token}` } };
  ...
  const handleSubmit = e => {
    ...
    if (mode === "add") {
      const config = { url: "/api/task", method: "post", data: formData, headers: { Authorization: `Bearer ${authState.token}` } };
      fetchData(config).then(() => {
        navigate("/");
      });
    } else {
      const config = { url: `/api/task/${taskId}`, method: "put", data: formData, headers: { Authorization: `Bearer ${authState.token}` } };
      fetchData(config).then(() => {
        navigate("/");
      });
    }
